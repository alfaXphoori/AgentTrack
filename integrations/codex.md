# Codex CLI Integration for AgentTrack
...
`atrack auto "<user_question>" "<ai_summary>" "codex-cli" 0 0`
...
2. When you run `codex` inside this directory, it will read `AGENTS.md` and adhere to the AgentTrack auto-logging requirements.
...
1. Create `~/.codex/prompts/log.md`:
   ```markdown
   ---
   description: Generate a AgentTrack log command
   ---
   Generate the exact shell command to log the recent session using AgentTrack:
   `atrack auto "<user_question>" "<ai_summary>" "codex-cli" 0 0`. 
