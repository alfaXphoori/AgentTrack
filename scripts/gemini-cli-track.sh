#!/bin/bash
# gemini-watch.sh - Background watcher: auto-logs native Gemini CLI sessions to TrackCLI
# Usage:  ./gemini-watch.sh          (foreground, Ctrl+C to stop)
#         ./gemini-watch.sh &         (background daemon)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TRACKCLI_BIN="$SCRIPT_DIR/trackcli"
PROJECT_DIR="$(pwd)"
POLL_INTERVAL=2
STATE_DIR="$PROJECT_DIR/.trackcli_watch_state"
mkdir -p "$STATE_DIR"
...
printf "\033[1;32m🔍 TrackCLI Gemini Watcher started\033[0m\n"
printf "   Watching: %s\n" "$CHATS_DIR"
...
process_session() {
  ...
  python3 - "$FILE" "$LOGGED" "$TRACKCLI_BIN" "$STATE_FILE" << 'PYEOF'
...
for p in new_pairs:
    ...
    tags = "gemini-cli"
    cmd = [trackcli, 'auto', q, summary, m, str(ti), str(to), str(dur), session_id, 'success', tools, tags]
